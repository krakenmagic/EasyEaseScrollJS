document.addEventListener("DOMContentLoaded", () => {
  ...
  const bounceThreshold = 0.005; // 0.01 = 1vh
 ...
 const handleScroll = () => {
  ...
  const viewportHeight = window.innerHeight;
  const threshold = viewportHeight * bounceThreshold; // 1vh相当などの値

  console.log(`handleScroll called: scrollY = ${scrollY}, viewportHeight = ${viewportHeight}, threshold = ${threshold}`);

  // スナップ処理
  // Bへ移行する条件: scrollY > threshold && scrollY < viewportHeight - threshold
  // Aへ戻る条件を強化して、Bの下部から戻ってきた場合にもAへ戻るようにする
  // ここで、scrollYがviewportHeightより下（B領域の下部）から上方向に戻って来て
  // viewportHeight - threshold を下回った瞬間にAへ戻るアニメーションを発動
  if (scrollY > threshold && scrollY < viewportHeight - threshold) {
    console.log("Snapping to B");
    smoothScrollTo(viewportHeight);
  } else if (scrollY < viewportHeight*1) {
